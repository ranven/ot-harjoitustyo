...
class TestProjectRepository(unittest.TestCase):
    def setUp(self):
        ...
        self.projects = [Project(title="testing", created_at=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')),
                         Project(title="cleaning", created_at=datetime.datetime.now().strftime(
                             '%Y-%m-%d %H:%M:%S')),
    ...
    def test_create(self):
        ...
        self.assertEqual(all_projects[0].created_at,
                         self.projects[0].created_at)
    ...
    def test_find_all(self):
        ...
        for p in self.projects:
            ...
            self.assertEqual(
                all_projects[i].created_at, self.projects[i].created_at)
            i += 1
